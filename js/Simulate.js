...
Actions.prototype.init = function()
{
	...
	//this.addAction('exportVerilog', function()
		//if( !runDRC(graph) )
		//{
			//mxLog.debug("Circuit has errors.  Please run a design rules check (DRC).");
			//mxLog.show();
			//return;
		//}
	this.addAction('exportVerilog', mxUtils.bind(this, function()
	{
...
*/
	...
	this.addAction('designRulesCheck', mxUtils.bind(this, function()
	{
		if (this.DRCWindow == null)
		{
			...
			//});
			//this.DRCWindow.window.addListener('hide', function()
			//{
				//ui.fireEvent(new mxEventObject('drc'));
			//});
		...
		runDRC(graph,true,this.DRCWindow);
	}), null, null, null);

/*the main simulator */

	this.addAction('simulate', function()
	{
		switch(operatingmode)
		{
		case "firstrun":
			var overlay_on = new mxCellOverlay(new mxImage('images/switchon.png',20,40),
				'Click to turn off',mxConstants.ALIGN_LEFT,mxConstants.ALIGN_MIDDLE);
			var overlay_off = new mxCellOverlay(new mxImage('images/switchoff.png',20,40),
				'Click to turn on',mxConstants.ALIGN_LEFT,mxConstants.ALIGN_MIDDLE);
			overlay_off.addListener(mxEvent.CLICK, function(sender, evt)
			{
				var cell=evt.getProperty("cell");
				graph.removeCellOverlays(cell);		
				graph.addCellOverlay(nodes[0], overlay_on);
			});
			overlay_on.addListener(mxEvent.CLICK, function(sender, evt)
			{
				var cell=evt.getProperty("cell");
				graph.removeCellOverlays(cell);		
				graph.addCellOverlay(nodes[0], overlay_off);
			});
			//continue on to editing
		case "editing":
	...
	{
		deleteCells(alwaysDeleteEdges || evt != null && mxEvent.isShiftDown(evt));
	}, null, null, 'Delete');
